...
export default async function handler(req, res) {
  try {
    ...
    for (const r of records) {
      ...
      const parentPresent = r.parent_present ? 1 : 0
      values.push('(?, ?, ?, ?, ?, NOW())')
      params.push(assignmentId, studentId, parentPresent, status, session.user.id)
    }
    ...
    return res.status(200).json({ message: 'Attendance saved', count: values.length })
  } catch (err) {
