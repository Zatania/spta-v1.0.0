...
export default async function handler(req, res) {
  try {
    ...
    if (fmt === 'csv') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="payments_sy_${syId}.csv"`)
      res.write(['assignment_id', 'activity', 'date', 'grade', 'section', 'lrn', 'last', 'first', 'status', 'paid', 'amount', 'payment_date'].map(csv).join(',') + '\n')
      for (const r of rows) {
        res.write([r.assignment_id, r.activity_title, r.activity_date, r.grade_name, r.section_name, r.lrn, r.last_name, r.first_name, r.payment_status, r.payment_paid, r.amount, r.payment_date].map(csv).join(',') + '\n')
      }
      res.end()
    ...
    if (fmt === 'xlsx') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="payments_sy_${syId}.xlsx"`)
      const workbook = new ExcelJS.stream.xlsx.WorkbookWriter({ stream: res })
      const sheet = workbook.addWorksheet('Payments')
      sheet.addRow(['Assignment ID', 'Activity', 'Date', 'Grade', 'Section', 'LRN', 'Last Name', 'First Name', 'Status', 'Paid', 'Amount', 'Payment Date']).commit()
      for (const r of rows) {
        sheet.addRow([r.assignment_id, r.activity_title, r.activity_date, r.grade_name, r.section_name, r.lrn, r.last_name, r.first_name, r.payment_status, Number(r.payment_paid), Number(r.amount || 0), r.payment_date]).commit()
      }
    ...
    if (fmt === 'pdf') {
      ...
      res.setHeader('Content-Disposition', `attachment; filename="payments_sy_${syId}.pdf"`)
      const doc = new PDFDocument({ margin: 40, size: 'A4' })
      doc.pipe(res)
      doc.fontSize(14).text(`Payments Export - School Year ${syId}`)
      doc.moveDown()
      doc.fontSize(9)
      for (const r of rows) {
        doc.text(`${r.activity_date} | ${r.activity_title} | ${r.grade_name}-${r.section_name} | ${r.lrn} ${r.last_name}, ${r.first_name} | ${r.payment_status} | ₱${Number(r.amount || 0).toFixed(2)}`)
